...
class FlowerClientCIFAR(fl.client.NumPyClient):
    ...
    def fit(
        ...
        train_loader: DataLoader = self._create_data_loader(config, name="train")
        val_loader: DataLoader = self._create_data_loader(config, name="val")
        train_loss, train_metrics = self._train(net, train_loader=train_loader, val_loader=val_loader, config=config)
        return get_model_parameters(net), len(train_loader), train_metrics
    ...
    def _train(
        self, net: Module, train_loader: DataLoader, val_loader: DataLoader, config: dict[str, Scalar]
    ) -> float:
        return train_CIFAR(
            net=net,
            train_loader=train_loader,
...
def get_flower_client_generator_cifar(
    model_generator: Callable[[], Module],
    partition_dir: Path,
    data_dir: Path,
    mapping_fn: Callable[[int], int] | None = None,
